 ...
 * @param y_src 输入图像中的y坐标
 */
void assignPixel(const Mat& input_image, Mat& output_image, int x_dst, int y_dst, int x_src, int y_src) {
    // 检查图像的通道数
 ...
 * @return Mat 缩放后的图像
 */
Mat nearestNeighborResizeBasic(const Mat& input_image, int output_width, int output_height) {
    ...
    for (int y_dst = 0; y_dst < output_height; ++y_dst) {
        for (int x_dst = 0; x_dst < output_width; ++x_dst) {
            ...
            // 赋值像素值到输出图像
            assignPixel(input_image, output_image, x_dst, y_dst, x_src, y_src);
        }
    }
...
int main() {
    ...
    // 执行最近邻插值缩放
    Mat resized_img = nearestNeighborResizeBasic(input_img, new_width, new_height);
