...
/**
 * @brief 使用最近邻插值算法对图像进行缩放，支持任意缩放因子和不同的宽高比
 * 
 ...
 * @return Mat 缩放后的图像
 */
Mat nearestNeighborResizeArbitrary(const Mat& input_image, int output_width, int output_height) {
...
/**
 * @brief 示例主函数，展示如何使用最近邻插值进行任意缩放的图像处理
 */
int main() {
    // 读取输入图像（可以是灰度图像或彩色图像）
    ...
    // 定义输出尺寸（可以是任意尺寸，不保持原始宽高比）
    int new_width = 800;  // 可以调整为任意值，实现上采样或下采样
    int new_height = 600; // 可以调整为任意值，实现上采样或下采样

    // 执行最近邻插值缩放，支持任意缩放因子和不同的宽高比
    Mat resized_img = nearestNeighborResizeArbitrary(input_img, new_width, new_height);

    // 保存缩放后的图像
    imwrite("resized_image_arbitrary.jpg", resized_img);

    // 可选：显示原始和缩放后的图像
    /*
    imshow("原始图像", input_img);
    imshow("缩放后图像 - 任意缩放因子", resized_img);
    waitKey(0);
    */

    cout << "图像缩放完成，保存到 resized_image_arbitrary.jpg" << endl;
    return 0;
}
